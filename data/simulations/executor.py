...
output_dir = '/root/data/simulations/results'
...
systems = {system['cores']: system for system in parameters['systems']}
executables = [f'{bin_path}/{executable}' for executable in parameters['executables']]

for grain_size in grain_sizes:
    for threads in thread_counts:
        system = systems[threads]
        for exec in executables:
            output = f'{output_dir}/sniper_{threads}_{grain_size}_{re.findall("sniper_(.+)$", exec)[0]}'
            ...
            with open(f'{output}/stdout.log', "w") as out:
                ...
                subprocess.run([f'{sniper_path}/run-sniper', 
                            ...
                            '--', exec, str(threads), str(iterations), str(grain_size)],
                            stdout=out)